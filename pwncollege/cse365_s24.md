...
> [!Note]
> It is because syscalls clobber the `r11` register.
...
mov r12, rax    # r12: Specified file FD

# Read 256 bytes from filepath into stack
mov rax, 0
mov rdi, r12
mov rsi, rsp
mov rdx, 256
...
# Write data read to connected socket
mov rax, 1
mov rsi, rsp
mov rdx, r12
syscall
...
Specificially, the level wants `accept(3, NULL, NULL)`.
```assembly
...
# Check if child process or parent process
cmp rax, 0
je child
```
...
Specifically, the level wants this
```
